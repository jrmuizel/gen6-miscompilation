...
#define NV_ANDROID_EXTERNAL() !defined(GTI_PROJECT) && defined(__ANDROID__) && !defined(__MINKE__)
...
struct DATABASE_HANDLE
{
    ...
    DATABASE_HANDLE& operator=(const DATABASE_HANDLE& other)
    {
...
public:
    ...
    //------------------------------------------------------------------------------
    // SetFileName- this indicates where we want to store results, if any.  If any previous
    // results are not finalized, they will be lost.  Passing NULL will result in a temporary
    // file, that will be deleted along with this object.
    //------------------------------------------------------------------------------
    ...
    //------------------------------------------------------------------------------
    NV_REPLAY_EXPORT uint64_t GetSize(const DATABASE_HANDLE& handle) override final;

    // Helpers for Read - Lock will return null if a page cannot be found
    NV_REPLAY_EXPORT DataScope::LockedPageHandle Lock(uint64_t pageOffset) override final;
    NV_REPLAY_EXPORT void Unlock(DataScope::LockedPageHandle pPageHandle) override final;
...
protected:
    ...
    bool InitReadOnly(const char* pFileNameReadOnly);
    ...
    // Helpers for Read
    NV_REPLAY_EXPORT void* DoRead(const DATABASE_HANDLE& handle) override final;
    NV_REPLAY_EXPORT void* DoRead(const DATABASE_HANDLE& handle, DataScopeTracker& scopeTracker) override final;
