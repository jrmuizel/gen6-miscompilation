...
public:
    ...
    virtual void CreateNativeWindow(void* data, int width, int height) override
    {
        ...
        if (data) {
            ...
            glXGetFBConfigAttrib(m_XDisplay, config, GLX_X_VISUAL_TYPE, &visualType);
            const bool writableColormap = (visualType != GLX_STATIC_GRAY && visualType != GLX_STATIC_COLOR && visualType != GLX_TRUE_COLOR);
